...
const regexMonthsLong =
  "January|February|March|April|May|June|July|August|September|October|November|December";
const regexWeeksLong =
...
class _utility {
  String = {
    ...
    split(value, separator, removeEmpty = false) {
      ...
      for (
        let i = 0;
        i < retval.length;
        i++ //Substitute special character with single separator instance
      )
        retval[i] = retval[i].replace(/\x07/g, separator);
      if (removeEmpty)
        //Remove empty splits (if asked for)
  ...
  Number = {
    format(value, mask) {
      function formatNumber(mask, value) {
        ...
        let mskInteger =
          indexDecimalPoint === -1 ? mask : mask.substr(0, indexDecimalPoint); //Extract integer part of mask
        let mskFraction =
          indexDecimalPoint === -1
            ? ""
            : mask.substr(
                indexDecimalPoint + 1,
                mask.length - indexDecimalPoint
              ); //Extract fraction part of mask
        let flgZero = false,
          flgHash = false,
          flgComma = false,
          flgDecimal = false;
        for (i = 0; i < mskInteger.length; i++) {
          ...
          if (c === ".") {
            //decimal
            if (flgDecimal) return ""; //There cannot be two decimal places
            if (flgComma) return ""; //Comma cannot preceed decimal
            flgDecimal = true; //flag that decimal point has been detected
          } else if (c === ",") {
            //comma
            if (flgComma) return ""; //There cannot be two consecutive commas
            if (flgDecimal) return ""; //Fraction portion cannot contain comma
            flgComma = true;
          } else if (c === "#") {
            //hash
            if (flgDecimal) return ""; //Fraction portion cannot have hash
            if (flgZero) return ""; //Hash cannot appear after Zeros
        ...
        if (!Number.isFinite(numIntegerDigitCount)) numIntegerDigitCount = 1; //Zero to be handled as single digit
        retval = numInteger.toString(); //First convert integer part to string
        if (numIntegerDigitCount > numIntegerTotalCount)
          //Pad extra has to the left (if mask falls short)
          mskInteger =
            "#".repeat(numIntegerDigitCount - numIntegerTotalCount) +
            mskInteger;
        if (numIntegerDigitCount < numIntegerTotalCount)
          //Pad extra zeros to the left (if required)
          retval =
            "0".repeat(numIntegerTotalCount - numIntegerDigitCount) + retval;
        ...
        for (i = 0; i < mskInteger.length; i++) {
          ...
          if (chMask === "0" || (chMask === "#" && cInteger > 48)) break;
          else stripCount++;
        }
      ...
      if (mask.includes(";")) {
        //Handle positive-negative-neutral
        let parts = mask.split(";");
        if (sign == 1) mask = parts[0];
        else if (sign == -1) mask = parts[1];
        else mask = parts.length == 3 ? parts[2] : parts[0];
      }
    ...
    round(value, precision = 0) {
      ...
      if (_value - Math.trunc(_value) > 0.5) _value = Math.trunc(_value) + 1;
      else _value = Math.trunc(_value);
  ...
  Date = {
    ...
    format(value, mask) {
      ...
      //Year replacement
      if (/yyyy/.test(mask)) mask = mask.replace("yyyy", vYear.toString());
      else if (/yy/.test(mask))
        mask = mask.replace("yy", vYear.toString().substr(2, 3));
      else;
      //Month replacement
      if (/MMMM/.test(mask)) mask = mask.replace("MMMM", monthsLong[vMonth]);
      ...
      //Day replacement
      if (/dddd/.test(mask)) mask = mask.replace("dddd", weeksLong[vWeek]);
      else if (/ddd/.test(mask)) mask = mask.replace("ddd", weeksShort[vWeek]);
      else if (/dd/.test(mask))
        mask = mask.replace("dd", vDay.toString().padStart(2, "0"));
      else if (/d/.test(mask)) mask = mask.replace("d", vDay.toString());
      else;
      //Hour replacement
      if (/HH/.test(mask))
        mask = mask.replace("HH", vHour.toString().padStart(2, "0"));
      else if (/H/.test(mask)) mask = mask.replace("H", vHour.toString());
      else if (/hh/.test(mask))
        mask = mask.replace(
          "hh",
          (vHour % 12 ? 0 : vHour % 12).toString().padStart(2, "0")
        );
      else if (/h/.test(mask))
      ...
      else if (/m/.test(mask)) mask = mask.replace("m", vMinute.toString());
      else;
      //Second replacement
      if (/ss/.test(mask))
        mask = mask.replace("ss", vSecond.toString().padStart(2, "0"));
      else if (/s/.test(mask)) mask = mask.replace("s", vSecond.toString());
      else;
      //AM-PM replacement
      if (/tt/.test(mask))
    ...
    parse(value, format) {
      ...
      parts = format.split(
        /(dddd|ddd|dd|d|MMMM|MMM|MM|M|yyyy|yy|HH|H|hh|h|mm|m|ss|s|tt|t)/g
      );
      for (
        let i = parts.length - 1;
        i >= 0;
        i-- //Remove empty parts
      )
      ...
      for (let i = 0; i < parts.length; i++) {
        let capExpr = parts[i];
        if (capExpr == "d") capExpr = "[1-9]|[1-2][0-9]|3[0-1]";
        else if (capExpr == "dd") capExpr = "0[1-9]|[1-2][0-9]|3[0-1]";
        else if (capExpr == "ddd") capExpr = regexWeeksShort;
        else if (capExpr == "dddd") capExpr = regexWeeksLong;
        else if (capExpr == "M") capExpr = "1[0-2]|[1-9]";
        else if (capExpr == "MM") capExpr = "0[1-9]|1[0-2]";
        else if (capExpr == "MMM") capExpr = regexMonthsShort;
        else if (capExpr == "MMMM") capExpr = regexMonthsLong;
        else if (capExpr == "yy") capExpr = "\\d{2}";
        else if (capExpr == "yyyy") capExpr = "\\d{4}";
        else if (capExpr == "H") capExpr = "[1-9]|1[0-9]|2[0-3]";
        else if (capExpr == "HH") capExpr = "0[0-9]|1[0-9]|2[0-3]";
        else if (capExpr == "h") capExpr = "[1-9]|1[0-2]";
        else if (capExpr == "hh") capExpr = "0[1-9]|1[0-2]";
        else if (capExpr == "m") capExpr = "[1-9]|[1-5][0-9]";
        else if (capExpr == "mm") capExpr = "[0-5][0-9]";
        else if (capExpr == "s") capExpr = "[1-9]|[1-5][0-9]";
        else if (capExpr == "ss") capExpr = "[0-5][0-9]";
        else if (capExpr == "tt") capExpr = "a\\.?m\\.?|A\\.?M\\.?";
        else;
        regexValue += `(${capExpr})`;
      }
      regexValue += "$";
      ...
      if (partResult.length !== parts.length + 1) return null; //Regex group match gives original value as extra (so ignored)
      for (let i = 0; i < parts.length; i++) {
        let partFormat = parts[i];
        let partValue = partResult[i + 1];
        let val = parseInt(partValue);
        if (partFormat == "d" || partFormat == "dd") vDay = val;
        //else if (partFormat == 'ddd') vMonth = weeksShort.indexOf(partValue);
        //else if (partFormat == 'dddd') vMonth = weeksLong.indexOf(partValue);
        else if (partFormat == "M" || partFormat == "MM") vMonth = --val;
        else if (partFormat == "MMM") vMonth = monthsShort.indexOf(partValue);
        else if (partFormat == "MMMM") vMonth = monthsLong.indexOf(partValue);
        else if (partFormat == "yy" || partFormat == "yyyy") vYear = val;
        else if (partFormat == "H" || partFormat == "HH") vHour = val;
        //else if (partFormat == 'h' || partFormat == 'hh') vHour = val; //am-pm handling to be impleamented
        else if (partFormat == "m" || partFormat == "mm") vMinute = val;
        else if (partFormat == "s" || partFormat == "ss") vSecond = val;
        else;
      }
      //2 digit year handling
      if (vYear < 100) vYear += vYear > 71 ? 1900 : 2000; //upto 70 in 1900 centurary
  ...
  Array = {
    distinct(source, field) {
      ...
      for (let i = 0; i < source.length; i++)
        if (!retval.includes(source[i][field])) retval.push(source[i][field]);
      return retval;
    },
    unique(arr, prop) {
      let retval = [];
